...
const MANIPULATION_RULES: DetectionRule[] = [
  ...
  },
  {
    ...
    pattern:
      /你(是|算)?(不是)?\s*(AI|ai|GPT|chatgpt|claude|deepseek|chatbot|bot|机器人|模型|大模型|llm|助手|agent)|are\s*you\s*(an?\s*)?(ai|bot|chatbot|llm|gpt|model|assistant)|背后是什么|你是谁写的|你是用什么(做|搭|写)的|prompt\s*是什么|你的\s*(system\s*)?prompt/i,
    weight: 3,
  },
